...
import static com.mindlibrary.images.MindLibImage.Priority.HIGH;
import static com.mindlibrary.images.MindLibImage.Priority.LOW;
import static com.mindlibrary.images.MindLibImage.Priority.NORMAL;
import static com.mindlibrary.images.TestUtils.ASSET_KEY_1;
import static com.mindlibrary.images.TestUtils.ASSET_URI_1;
import static com.mindlibrary.images.TestUtils.CONTACT_KEY_1;
import static com.mindlibrary.images.TestUtils.CONTACT_PHOTO_KEY_1;
import static com.mindlibrary.images.TestUtils.CONTACT_PHOTO_URI_1;
import static com.mindlibrary.images.TestUtils.CONTACT_URI_1;
import static com.mindlibrary.images.TestUtils.CONTENT_1_URL;
import static com.mindlibrary.images.TestUtils.CONTENT_KEY_1;
import static com.mindlibrary.images.TestUtils.CUSTOM_URI;
import static com.mindlibrary.images.TestUtils.CUSTOM_URI_KEY;
import static com.mindlibrary.images.TestUtils.FILE_1_URL;
import static com.mindlibrary.images.TestUtils.FILE_KEY_1;
import static com.mindlibrary.images.TestUtils.MEDIA_STORE_CONTENT_1_URL;
import static com.mindlibrary.images.TestUtils.MEDIA_STORE_CONTENT_KEY_1;
import static com.mindlibrary.images.TestUtils.RESOURCE_ID_1;
import static com.mindlibrary.images.TestUtils.RESOURCE_ID_KEY_1;
import static com.mindlibrary.images.TestUtils.RESOURCE_ID_URI;
import static com.mindlibrary.images.TestUtils.RESOURCE_ID_URI_KEY;
import static com.mindlibrary.images.TestUtils.RESOURCE_TYPE_URI;
import static com.mindlibrary.images.TestUtils.RESOURCE_TYPE_URI_KEY;
...
import static com.mindlibrary.images.TestUtils.mockPicasso;
...
public class BitmapHunterTest {
  ...
  @Test public void nullDecodeResponseIsError() {
    ...
    BitmapHunter hunter = new TestableBitmapHunter(mindLibImage, dispatcher, cache, stats, action, null);
    hunter.run();
  ...
  @Test public void runWithResultDispatchComplete() {
    Action action = mockAction(URI_KEY_1, URI_1);
    BitmapHunter hunter = new TestableBitmapHunter(mindLibImage, dispatcher, cache, stats, action, bitmap);
    hunter.run();
  ...
  @Test public void runWithNoResultDispatchFailed() {
    Action action = mockAction(URI_KEY_1, URI_1);
    BitmapHunter hunter = new TestableBitmapHunter(mindLibImage, dispatcher, cache, stats, action);
    hunter.run();
    verify(dispatcher).dispatchFailed(hunter);
  }
  ...
  @Test public void outOfMemoryDispatchFailed() {
    ...
    Action action = mockAction(URI_KEY_1, URI_1);
    BitmapHunter hunter = new OOMBitmapHunter(mindLibImage, dispatcher, cache, stats, action);
    try {
      hunter.run();
    } catch (Throwable t) {
  ...
  @Test public void huntDecodesWhenNotInCache() throws Exception {
    ...
    TestableBitmapHunter hunter =
        new TestableBitmapHunter(mindLibImage, dispatcher, cache, stats, action, bitmap);
  ...
  @Test public void huntUnrecognizedUri() throws Exception {
    ...
    BitmapHunter hunter = BitmapHunter.forRequest(mindLibImage, dispatcher, cache, stats, action);
    try {
      hunter.hunt();
  ...
  @Test public void huntDecodesWithRequestHandler() throws Exception {
    Action action = mockAction(CUSTOM_URI_KEY, CUSTOM_URI);
    BitmapHunter hunter = BitmapHunter.forRequest(mockPicasso(new CustomRequestHandler()), dispatcher,
        cache, stats, action);
    Bitmap result = hunter.hunt();
    assertThat(result).isEqualTo(bitmap);
  }
  ...
  @Test public void attachSingleRequest() {
    ...
    BitmapHunter hunter = new TestableBitmapHunter(mindLibImage, dispatcher, cache, stats, action1);
  ...
  @Test public void forContentProviderRequest() {
    ...
    BitmapHunter hunter = BitmapHunter.forRequest(mockPicasso(new ContentStreamRequestHandler(context)),
  ...
  @Test public void forMediaStoreRequest() {
    ...
    BitmapHunter hunter = BitmapHunter.forRequest(mockPicasso(new MediaStoreRequestHandler(context)), dispatcher,
        cache, stats, action);
  ...
  @Test public void forContactsPhotoRequest() {
    ...
    BitmapHunter hunter = BitmapHunter.forRequest(mockPicasso(new ContactsPhotoRequestHandler(context)),
        dispatcher, cache, stats, action);
  ...
  @Test public void forNetworkRequest() {
    Action action = mockAction(URI_KEY_1, URI_1);
    BitmapHunter hunter = BitmapHunter.forRequest(mockPicasso(new NetworkRequestHandler(downloader, stats)),
        dispatcher, cache, stats, action);
  ...
  @Test public void forFileWithAuthorityRequest() {
    ...
    BitmapHunter hunter = BitmapHunter.forRequest(mockPicasso(new FileRequestHandler(context)), dispatcher,
        cache, stats, action);
  ...
  @Test public void forAndroidResourceRequest() {
    ...
    BitmapHunter hunter = BitmapHunter.forRequest(mockPicasso(new ResourceRequestHandler(context)), dispatcher,
        cache, stats, action);
  ...
  @Test public void forAssetRequest() {
    ...
    BitmapHunter hunter = BitmapHunter.forRequest(mockPicasso(new AssetRequestHandler(context)), dispatcher,
        cache, stats, action);
  ...
  @Test public void forCustomRequest() {
    Action action = mockAction(CUSTOM_URI_KEY, CUSTOM_URI);
    BitmapHunter hunter = BitmapHunter.forRequest(mockPicasso(new CustomRequestHandler()), dispatcher, cache,
        stats, action);
  ...
  @Test public void forOverrideRequest() {
    ...
    // Must use non-mock constructor because that is where MindLibImage's list of handlers is created.
    MindLibImage mindLibImage = new MindLibImage(context, dispatcher, cache, null, null, handlers, stats,
  ...
  @Test public void sequenceIsIncremented() {
    ...
    BitmapHunter hunter1 = BitmapHunter.forRequest(mindLibImage, dispatcher, cache, stats, action);
    BitmapHunter hunter2 = BitmapHunter.forRequest(mindLibImage, dispatcher, cache, stats, action);
  ...
  @Test public void exifRotation() {
    ...
    Bitmap result = BitmapHunter.transformResult(data, source, ORIENTATION_ROTATE_90);
 ...
 @Test public void rotation90Sizing() throws Exception {
    ...
    Bitmap source = Bitmap.createBitmap(10, 10, ARGB_8888);
    Bitmap result = BitmapHunter.transformResult(data, source, 0);
    ShadowBitmap shadowBitmap = shadowOf(result);
    assertThat(shadowBitmap.getCreatedFromBitmap()).isSameAs(source);
 ...
 @Test public void rotation90WithPivotSizing() throws Exception {
  ...
  @Test public void exifVerticalFlip() {
    Request data = new Request.Builder(URI_1).rotate(-45).build();
    Bitmap source = Bitmap.createBitmap(10, 10, ARGB_8888);
    Bitmap result = BitmapHunter.transformResult(data, source, ORIENTATION_FLIP_VERTICAL);
    ShadowBitmap shadowBitmap = shadowOf(result);
    assertThat(shadowBitmap.getCreatedFromBitmap()).isSameAs(source);
  ...
  @Test public void exifHorizontalFlip() {
    Request data = new Request.Builder(URI_1).rotate(-45).build();
    Bitmap source = Bitmap.createBitmap(10, 10, ARGB_8888);
    Bitmap result = BitmapHunter.transformResult(data, source, ORIENTATION_FLIP_HORIZONTAL);
    ShadowBitmap shadowBitmap = shadowOf(result);
    assertThat(shadowBitmap.getCreatedFromBitmap()).isSameAs(source);
  ...
  @Test public void exifTranspose() {
    Request data = new Request.Builder(URI_1).rotate(-45).build();
    Bitmap source = Bitmap.createBitmap(10, 10, ARGB_8888);
    Bitmap result = BitmapHunter.transformResult(data, source, ORIENTATION_TRANSPOSE);
    ShadowBitmap shadowBitmap = shadowOf(result);
    assertThat(shadowBitmap.getCreatedFromBitmap()).isSameAs(source);
  ...
  @Test public void exifTransverse() {
    Request data = new Request.Builder(URI_1).rotate(-45).build();
    Bitmap source = Bitmap.createBitmap(10, 10, ARGB_8888);
    Bitmap result = BitmapHunter.transformResult(data, source, ORIENTATION_TRANSVERSE);
    ShadowBitmap shadowBitmap = shadowOf(result);
    assertThat(shadowBitmap.getCreatedFromBitmap()).isSameAs(source);
  ...
  @Test public void keepsAspectRationWhileResizingWhenDesiredWidthIs0() {
    ...
    Bitmap result = BitmapHunter.transformResult(request, source, 0);
  ...
  private static class TestableBitmapHunter extends BitmapHunter {
    TestableBitmapHunter(MindLibImage mindLibImage, Dispatcher dispatcher, Cache cache, Stats stats,
                         Action action) {
      this(mindLibImage, dispatcher, cache, stats, action, null);
    }

    TestableBitmapHunter(MindLibImage mindLibImage, Dispatcher dispatcher, Cache cache, Stats stats,
                         Action action, Bitmap result) {
      this(mindLibImage, dispatcher, cache, stats, action, result, null);
    }

    TestableBitmapHunter(MindLibImage mindLibImage, Dispatcher dispatcher, Cache cache, Stats stats,
                         Action action, Bitmap result, IOException exception) {
      super(mindLibImage, dispatcher, cache, stats, action, spy(new TestableRequestHandler(result, exception)));
    }

    @Override MindLibImage.LoadedFrom getLoadedFrom() {
      return MEMORY;
    }
  }
  ...
  private static class OOMBitmapHunter extends BitmapHunter {
    OOMBitmapHunter(MindLibImage mindLibImage, Dispatcher dispatcher, Cache cache, Stats stats,
                    Action action) {
      super(mindLibImage, dispatcher, cache, stats, action, spy(new OOMRequestHandler()));
    }
  }
