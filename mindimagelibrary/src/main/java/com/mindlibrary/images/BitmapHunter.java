...
import static com.mindlibrary.images.Utils.getLogIdsForHunter;
...
class BitmapHunter implements Runnable {
  ...
  Bitmap result;
  Future<?> future;
  MindLibImage.LoadedFrom loadedFrom;
  Exception exception;
  int exifOrientation; // Determined during decoding of original resource.
  int retryCount;
  Priority priority;

  BitmapHunter(MindLibImage mindLibImage, Dispatcher dispatcher, Cache cache, Stats stats, Action action,
  ...
  MindLibImage.LoadedFrom getLoadedFrom() {
    return loadedFrom;
  }
  ...
  static BitmapHunter forRequest(MindLibImage mindLibImage, Dispatcher dispatcher, Cache cache, Stats stats,
                                 Action action) {
    Request request = action.getRequest();
    List<RequestHandler> requestHandlers = mindLibImage.getRequestHandlers();
    ...
    for (int i = 0, count = requestHandlers.size(); i < count; i++) {
      ...
      if (requestHandler.canHandleRequest(request)) {
        return new BitmapHunter(mindLibImage, dispatcher, cache, stats, action, requestHandler);
      }
    }

    return new BitmapHunter(mindLibImage, dispatcher, cache, stats, action, ERRORING_HANDLER);
  }
  ...
  static Bitmap applyCustomTransformations(List<Transformation> transformations, Bitmap result) {
    for (int i = 0, count = transformations.size(); i < count; i++) {
      ...
      try {
        ...
        MindLibImage.HANDLER.post(new Runnable() {
          @Override public void run() {
